...
if(street.val()!==addressModel.selectedAddress()[field][index]){$(street).attr('value',addressModel.selectedAddress()[field][index]).trigger('change');}});this.updateFieldValue(form,'city');this.updateFieldValue(form,'region');this.updateFieldValue(form,'region_id');this.updateFieldValue(form,'country_id');this.updateFieldValue(form,'postcode');},updateFieldValue:function(form,field){var fieldElement=$(form).find("input[name*="+field+"]");if(fieldElement.val()!==addressModel.selectedAddress()[field]){$(fieldElement).attr('value',addressModel.selectedAddress()[field]).trigger('change');}},diffAddressField:function(o,n){o=this.encodeHtml(o);n=this.encodeHtml(n);if(o!==n){addressModel.isDifferent(true);if(n.length){n='<span class="address-field-changed">'+n+'</span>';}}
