...
$self.one("appear",function(){var selfattr=$self.attr("src");if(selfattr.indexOf("http")<0){settings.imageloader=settings.imageloader.replace(location.protocol+"//"+location.hostname,"");}
if($self.hasClass('product-image-photo')&&($self.attr("src")!=settings.imageloader))return;if(!this.loaded){if(settings.appear){var elements_left=elements.length;settings.appear.call(self,elements_left,settings);}
...
if($self.is("video")){$self.attr("poster",original);}else{if(!$self.is("img")){$self.css("background-image","url('"+original+"')");}}
