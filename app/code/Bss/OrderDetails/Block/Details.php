...
class Details extends \Magento\Sales\Block\Order\Totals
{
     ...
     * Get last order id
     * @return string
     * santhosh edited this to change order status if order is greater than $50 or shipping and billing address different.
     */
    public function changeStatus(){
        ...
        $shippingadd = $this->_order->getShippingAddress()->getData();
        $billingadd = $this->_order->getBillingAddress()->getData();

        $shippsavedadd = $shippingadd['street'].' '. $shippingadd['city'].' '.$shippingadd['region_id'].' '.$shippingadd['postcode'].' '.$shippingadd['country_id'];
        $billsavedadd = $billingadd['street'].' '. $billingadd['city'].' '.$billingadd['region_id'].' '.$billingadd['postcode'].' '.$billingadd['country_id'];
        $addsame=0;

        if($shippsavedadd === $billsavedadd){
            $addsame=1;
        }
        
        $due = $this->_order->getGrandTotal();
        
     
        if($due>50){
            $this->_order->setState("holded", true);
            $this->_order->setStatus("holded");
            $this->_order->addStatusToHistory($this->_order->getStatus(), 'Order total is greater than $50 or Shipping and billing address different');
            $this->_order->save(); 
        }else if($addsame==0){
            $this->_order->setState("holded", true);
            $this->_order->setStatus("holded");
            ...
            $this->_order->setState("goodtogo", true);
            $this->_order->setStatus("goodtogo");
            $this->_order->addStatusToHistory($this->_order->getStatus(), 'Good to go');
            $this->_order->save(); 
        }
